...
public class PaymentFragment extends Fragment {
    ...
    private ArrayList<ShoppingCart> selectedItems;
    private ProgressDialog pd;
    private final String TAG = "PaymentFragment";
    private float gtotal;
    private int item_count;

    public PaymentFragment(ArrayList<ShoppingCart> selectedItems) {
    ...
    private void placeOrder(String custid,String mode) {
        //Order management
        String order_id = dbRef.child("FashionOrders").child(custid).push().getKey();
        ...
        for (ShoppingCart cart : selectedItems){
            String details_id = dbRef.child("FashionOrders").child(custid).child(order_id).child("FashionDetails").push().getKey();
        ...
        dbRef.child("FashionOrders").child(custid).child(order_id).setValue(order); //saving the order

        //Payment management
        Payment payment = new Payment();
        String pay_id = dbRef.child("FashionPayments").child(custid).push().getKey();
        ...
        dbRef.child("FashionPayments").child(custid).child(pay_id).setValue(payment); //saving the payment
        ...
        for (ShoppingCart cart : selectedItems){
            ...
            dbRef.child("FashionCart").child(custid).child("FashionItems").child(cart_id).removeValue(); //remove cart entry

            //update stock
            dbRef.child("FashionStock").child(prod_id).child(size).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    int stock = Integer.parseInt(snapshot.getValue().toString());
                    int new_stock = stock-qty;
                    dbRef.child("FashionStock").child(prod_id).child(size).setValue(new_stock);
    ...
    private void getShippingAddress(String uid) {
        dbRef.child("FashionCustomer").child(uid).child("cust_address").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
    ...
    private void fetchCartData() {
        for(ShoppingCart cart : this.selectedItems){
